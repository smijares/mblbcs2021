...
# -*- coding: utf-8 -*-
"""
Neural network mblbcs2021
V3.3
Sebastià Mijares i Verdú - GICI, UAB
sebastia.mijares@uab.cat

Script template to train and run neural-network encoders.

This script is based on bls2017.py by J. Ballé.

How to use this code
--------------------

This script is run from a terminal. Use option -h to get help on the commands.

There are three functions in this script: train, compress, and decompress.

This script is only compatible with data in (.raw) format, stored as 16-bit unsigned integers, preferably in little endian byte order.
...
THIS IS HARD-CODED IN THE PARAMETER BELOW.
"""
...
class SynthesisTransform(tf.keras.Sequential):
  ...
  def __init__(self, num_filters,bands):
    ...
    self.add(tfc.SignalConv2D(
        bands, (9, 9), name="layer_2", corr=False, strides_up=4,
        padding="same_zeros", use_bias=True,
        activation=None))
    self.add(tf.keras.layers.Lambda(lambda x: x * ((2**bit_length)-1)))


class BLS2021Model(tf.keras.Model):
  """Main model class."""

  def __init__(self, lmbda, num_filters,bands):
    super().__init__()
    self.lmbda = lmbda
    self.bands = bands
    self.analysis_transform = AnalysisTransform(num_filters)
    self.synthesis_transform = SynthesisTransform(num_filters,bands)
  ...
  def decompress(self, string, x_shape, y_shape):
    ...
    return tf.saturate_cast(tf.round(x_hat), data_type)
...
def train(args):
  ...
  model = BLS2021Model(args.lmbda, args.num_filters, args.bands)
  model.compile(
...
def main(args):
  ...
  if args.command == "train":
    ...
    if not args.output_file:
      args.output_file = args.input_file + ".raw"
    decompress(args)
